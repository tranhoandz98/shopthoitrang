...
$catego = mysqli_query($conn, "SELECT * FROM category WHERE status=1");
if ($_GET['danh-muc']) {
	$slug = $_GET['danh-muc'];
	$category = mysqli_query($conn, "SELECT * FROM category WHERE slug='$slug'");
	$cate = mysqli_fetch_assoc($category);
	$id_cate = $cate['id'];
	$product = mysqli_query($conn, "SELECT * FROM product WHERE id_cate='$id_cate' and status =1");
	$pro = mysqli_fetch_assoc($product);
// b1 lấy tổng số bản ghi của mảng
	$total = mysqli_num_rows($product);
// Bước 2 thiết lập số bản ghi trên 1 mảng
	$limit = 3;
...
// bước 6 mở query sử dụng limit
	$query = mysqli_query($conn, "SELECT * FROM product WHERE id_cate='$id_cate' and status=1 ORDER BY id DESC LIMIT $star,$limit");
}
...
<div class="container">
  <div class="row">
   <div class="col-md-3">
	...
	?>
          <tr>
            <td><a href="category.php?danh-muc=<?php echo $value['slug'] ?>&page=1"><?php echo $value['name']; ?></a></td>
          </tr>
        <?php }?>
      </tbody>
    </table>
   ...
   <div class="container">
    <div class="row">
      ...
      <div class="col-md-9">
        <div class="panel panel-primary">
         <div class="panel-heading">
           <h3 class="panel-title">Sản phẩm theo danh mục: <?php echo $cate['name'] ?>
	...
	?>
            <div class="col-md-4 new-pro text-center">
              <div class="thumbnail">
                ...
                <?php }?>
                <p>
                  <a href="cart.php?id=<?php echo $value['id'] ?>" class="btn btn-primary" class="btn btn-primary">Mua hàng</a>
        ...
        <ul class="pagination pull-right">
          <?php if ($cr_page > 1) {?>
          <li><a href="category.php?danh-muc=<?php echo $cate['slug'] ?>&page=<?php echo ($cr_page - 1) ?>">&laquo;</a></li>
        <?php }?>
          <?php for ($i = 1; $i <= $page; $i++) {
	?>
            <li><a href="category.php?danh-muc=<?php echo $cate['slug'] ?>&page=<?php echo $i ?>"> <?php echo $i ?></a></li>
          <?php }?>
          <?php if ($cr_page < $page) {?>
          <li><a href="category.php?danh-muc=<?php echo $cate['slug'] ?>&page=<?php echo ($cr_page + 1) ?>">&raquo;</a></li>
        <?php }?>
        </ul>
    </div>
  </div>
